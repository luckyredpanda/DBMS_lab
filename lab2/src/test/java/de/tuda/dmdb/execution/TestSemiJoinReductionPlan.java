...
import de.tuda.dmdb.execution.exercise.SemiJoinReductionPlan;
...
public class TestSemiJoinReductionPlan extends TestCase {
  ...
  public void testSimple() throws InterruptedException {
    ...
    ordersRecord1.setValue(0, new SQLInteger(1));
    orders1.insert(ordersRecord1);

    // orders2 is empty so there are no join partners that node1 can find
    expectedResult2.add(
        customerRecord2); // orders1 contains one item so node2 should find one join partner

    int nodeId = 0;
    int listenerPort = 6300;
    ...
    Runnable task1 =
        () -> {
          ...
          SemiJoinReductionPlan localJoin =
              new SemiJoinReductionPlan(
                  tableScanCustomers,
                  tableScanOrders,
                  leftJoinAtt,
                  rightJoinAtt,
                  ...
                  listenerPort,
                  nodeMap);
          Operator root = localJoin.compilePlan();
    ...
    Assertions.assertEquals(
        expectedResult.size(), resultList.size(), "Result list should contain as many records: ");
    Assertions.assertEquals(
        expectedResult2.size(), resultList2.size(), "Result list should contain as many records: ");
